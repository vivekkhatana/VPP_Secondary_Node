...
class ConnectionHandler:
    ...
    def get_message(self):
        ...
        while True:
            try:
                data = self.sock.recv(1024)  # Get at max 512 bytes of data from the client
    ...
    def send_message_to_other_client(self, payload, sock1):
        """Prepares and sends a payload to the along the socket"""

        try:
            sock1.send(payload.encode("utf-8"))
            return True
        except socket.error:
            raise DisconnectionException("Failed to send message")         
